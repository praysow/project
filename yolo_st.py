...
def detect_objects_in_video(video_path, desired_classes):
    ...
    while cap.isOpened():
        ...
        # 감지된 라벨에 대해 TTS 처리
        if '빨간불' in detected_labels:
            if not tts_active.is_set():
                threading.Thread(target=speak_location_and_label, args=(location, '빨간불')).start()
        elif '초록불' in detected_labels:
            if not tts_active.is_set():
                threading.Thread(target=speak_location_and_label, args=(location, '초록불')).start()
        elif '횡단보도' in detected_labels:
            if not tts_active.is_set():
                threading.Thread(target=speak_location_and_label, args=(location, '횡단보도')).start()
        else:
            for label in detected_labels:
                if not tts_active.is_set():
                    threading.Thread(target=speak_location_and_label, args=(location, label)).start()
